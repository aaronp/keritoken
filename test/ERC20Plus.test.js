...
describe('ERC20Plus', function () {
  const POLICY_LABEL = 'test-policy-said';
  const POLICY_ID = ethers.keccak256(ethers.toUtf8Bytes(POLICY_LABEL));
  ...
  beforeEach(async function () {
    ...
    token = await TokenFactory.deploy('TestToken', 'TT', await registry.getAddress(), POLICY_LABEL);
    const minterRole = await token.MINTER_ROLE();
    await token.grantRole(minterRole, minter.address);
    await registry.setAllowed(POLICY_LABEL, alice.address, true);
    await registry.setAllowed(POLICY_LABEL, bob.address, true);
  });

  describe('Deployment', function () {
    it('stores policy registry and hashes label to policy id', async function () {
    ...
    it('rejects zero registry address', async function () {
      ...
      await expect(Factory.deploy('X', 'X', ethers.ZeroAddress, POLICY_LABEL)).to.be.revertedWith('Zero registry address');
    });
  ...
  describe('Minting', function () {
    ...
    it('rejects mint to ineligible address', async function () {
      await registry.setAllowed(POLICY_LABEL, bob.address, false);
  ...
  describe('Transfer', function () {
    ...
    it('rejects transfer from ineligible sender', async function () {
      await registry.setAllowed(POLICY_LABEL, alice.address, false);
